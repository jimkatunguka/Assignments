...
public class Problem1 {
	...
	public static List<Customer> specialCustomers(List<Account> accts) {
		//implement
		return accts.stream()
				.filter(a->a.getCustomer().getCheckingAccount().getId().length()==a.getCustomer().getAddress().getCity().length())
				.map(a->a.getCustomer())
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		//implement 
		return custs.stream()
				.filter(c->c.getCheckingAccount().getBalance()<100 && c.getCheckingAccount().getBalance()>50 && c.getFirstName().charAt(0)=='A')
